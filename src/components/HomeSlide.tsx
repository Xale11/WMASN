...
const HomeSlide = () => {
  ...
  return (
    <Box w={"100%"} bg={"#2c2c2c"} transform={"scale(1)"} h={{base: "25vh", sm: "calc(100vh - 7em)", md: "50vh", lg: "40vh", xl: "calc(100vh - 8em)", "2xl": "calc(100vh - 6em)"}} display={"flex"} position={"relative"}  backgroundSize={"cover"} backgroundRepeat={"no-repeat"} backgroundPosition={"center center"} zIndex={0} >
        ...
        <Box onClick={() => {navigate("/projects")}} w={"fit-content"} fontSize={{base: "0.7em", sm: "1em"}} position={"absolute"} borderRadius={"0em"} bg={"white"} display={"flex"} alignItems={"center"} gap={"0.5em"} justifyContent={"center"} padding={"0.5em 1.75em"} color={"#2c2c2c"} transition={"all 300ms ease-in-out"} cursor={"pointer"} _hover={{padding: "0.5em 2.5em"}} zIndex={4} top={{base: "77%", sm: "77%", md: "85%", xl:"80%"}} left={"0"} right={"0"} margin={"0 auto"}>
          <Text fontFamily={"Roboto"} letterSpacing={"3px"}>VIEW PROJECTS</Text>
